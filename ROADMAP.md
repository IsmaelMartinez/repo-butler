...
### Code Health Sprint — multi-agent simplification review

A four-team subagent review on 2026-04-28 surveyed the whole codebase (~12.5k LOC, 22 source files) and produced a prioritised simplification plan. Two correctness bugs surfaced, plus three drift hazards CLAUDE.md already flagged informally and roughly twenty mechanical dedupe opportunities. Each tier below can run as its own session — the items are independent across tiers.

Tier 0 (correctness, must-ship). Monitor cursor persistence is broken: `monitor.js loadCursor` returns `null` unconditionally and `saveCursor` checks `store.writeFile` which the store never exposed, so the every-6h monitor reports every open issue/PR/alert as new on every run and the council re-triages the same backlog forever. Watchlist persistence (`council.js loadWatchlist`/`saveWatchlist`) shares the same root cause. Both fixed by adding `readJSON`/`writeJSON` to the store factory and wiring the two callers through it. Shipped as a single PR (this entry will be moved to "Implemented" on merge).

Tier 1 (high-value structural cleanups). Unify the campaign definitions duplicated between `mcp.js` and `report-portfolio.js` (CLAUDE.md already calls out this drift risk). Extract one `buildRepoSnapshot(r, details, options)` helper to replace three drift-prone construction sites that all feed `computeHealthTier` (`report-repo.js snapshotToTierInput`, the inline `repoSnapshot` in `report.js`, and the synthetic snapshot in `report-portfolio.js buildPortfolioAttentionSection`). Reshape `src/index.js` from 286 LOC of phase-wiring + ad-hoc bodies into a thin dispatcher, with each phase exporting its own `runX(context)`; drop the cold-path `await import()` calls. Add `gh.putFile` / `gh.deleteFile` to `github.js` and route the five hand-rolled `/contents/{path}` blocks in `store.js` through them, plus an optional `{ ref }` param to `gh.getFileContent` / `gh.listDir` so `store.js` can drop its private `readFile`/`listBranchDir` re-implementations. Rough impact ~150 LOC removed.

Tier 2 (pattern dedup, real readability wins). Extract a `summariseAlerts(alerts, getSeverity)` helper to collapse the identical severity-tally bodies in `observe.js fetchDependabotAlerts` / `fetchCodeScanningAlerts` and the three near-identical scanner blocks in `monitor.js detectSecurityAlerts`. Add a `colorByThreshold(value, [{lt, color}, ...])` helper to `report-shared.js` — the value→colour pattern is open-coded across nine sites in `report-portfolio.js` and `report-repo.js`. Merge the two LLM providers into `providers/base.js fetchJson(url, headers, body, extractText)` since they're 90% identical. Extract a `wrapPrompt(role, projectContext, items, outroLines)` helper used by `assess.js`, `update.js`, `ideate.js`, and the three council prompt builders so the `PROMPT_DEFENCE` + `DATA_BOUNDARY_START` scaffolding is in one place — defence-in-depth as much as simplification. Rough impact ~190 LOC.

Tier 3 (local cleanups). Collapse the `mcp.js` 9-arm `callTool` switch by attaching `handler` to each tool entry. Replace the hardcoded persona list in `mcp.js` (the "to avoid dynamic import in sync context" comment is wrong — static ESM works fine) with `import { PERSONAS } from './council.js'`. Extract a `bucketVerdicts(items, verdicts, enrich)` helper for `triageEvents`/`reviewProposals` in `council.js`. Extract a `detectMetricDrift(eligible, getValue, threshold, category, format)` helper for the structurally-identical CI and community-health drift detectors in `governance.js`. Switch `observe.js fetchMergedPRs` from the search API to `gh.paginate('/pulls', { state: closed })` to align with CLAUDE.md's own "use list/paginate over search" rule. Extract `pruneDir(dirPath, max)` in `store.js`. Simplify the `parseIdeas` regex pyramid in `ideate.js` to a one-line match-all loop. Extract an `htmlPage({ title, head, body, charts })` template in `report-styles.js`. Extract `buildStatCard` for the eight repeated card fragments in `report-repo.js buildHealthSection`. Promote `.muted`, `.text-success`/`.text-warning`/`.text-danger` utility classes to replace the inline `style=""` repetition. Add a `gitShow(refSuffix, args)` helper in `mcp.js` for the duplicated `origin/repo-butler-data` → `repo-butler-data` fallback. Rough impact ~250 LOC.

Items deliberately not in the plan: `config.js` hand-rolled YAML parser (40 LOC, well-scoped to CLAUDE.md's flat + one-level-nested contract), `libyear.js` cohesion (single-purpose, single caller — folding into `report-portfolio.js` would mix registry concerns into reporting), and any further file splitting (the 5-file report split is intentional).
