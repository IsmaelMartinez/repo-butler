...
The six-phase pipeline has three phases wired to a trigger: OBSERVE, ASSESS, and REPORT run daily via `self-test.yml`, and MONITOR runs every 6h via `monitor.yml`. UPDATE, IDEATE, and PROPOSE are effectively manual-only. `.github/roadmap.yml:6-8` declares `schedule: { assess: daily, ideate: weekly }`; `assess: daily` is now honored, but `ideate: weekly` remains aspirational. The 2026-04-14 incident where `snapshots/latest.json` had been frozen since 2026-04-03 (because `self-test.yml` defaulted to `phase=report` with no OBSERVE) exposed this gap; the fix landed as commit `9795952` on main. The remaining work is to graduate the other phases onto real schedules.
