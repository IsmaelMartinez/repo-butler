...
**Portfolio policy definition** — Add a `standards` section to `.github/roadmap.yml` where the maintainer declares what every repo should have (e.g., Dependabot, issue form templates, MIT license, CodeRabbit). The butler compares each repo against these standards and flags non-compliance. Without explicit standards, the butler infers them from majority adoption — if 14/19 repos have Dependabot, it proposes adding it to the other 5.

**Standards propagation** — Detect when a tool or configuration is adopted in some repos but not all. Generate proposals to bring lagging repos into alignment. Examples: Dependabot configured in 5 repos but 14 missing. Issue form templates in 3 repos but 16 using old markdown format. CODEOWNERS added to 2 repos — the rest could benefit.
