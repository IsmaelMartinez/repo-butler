...
### The MCP staleness guard — recommended next (G8, #350)
...
The MCP server has the same shape, and three separate defects have now surfaced from it inside a week — enough to say what they have in common. `runGitOnDataBranch` never fetches, so it serves whatever `origin/repo-butler-data` last pointed at and goes stale without saying so, which on 2026-07-25 produced a briefing claiming 12 Gold against a true 7. `src/mcp.js` never loaded `.github/roadmap.yml`, so every tier it computed dropped the `release_exempt` list and reported two deliberately-exempt repos as Silver while the rest of the pipeline called them Gold — a phantom regression fixed in #359, guarded by a source-level test so a new call site cannot silently omit it. And `computeHealthTier` measures release age against `Date.now()`, so `get_weekly_trend` re-scores *archived* weekly snapshots with today's clock: a release that was thirty days old in W22 is scored past the 90-day Gold boundary today, and historical Gold counts decay as the snapshots age. That last one is not yet fixed and made the tier history in the 2026-07-30 retrospective wrong until it was re-read from the stored snapshots.

The common cause is that the server *re-derives* what it could *report*. Every weekly snapshot already carries `computed.tier` and its full checks array, written by `store.js` at the time with the exemption applied. So the shape of G8 is two changes that reinforce each other: read the stored tier for historical weeks rather than recomputing it, and carry an explicit staleness signal — the age of the data served, and how far the linked checkout is behind the remote default branch — warning rather than answering confidently past a threshold. The garden plan's acceptance criteria already cover the second half. The skills-installer half ships independently.
