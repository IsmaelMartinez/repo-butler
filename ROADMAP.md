...
**Butler-briefing/debrief refresh or retirement** — Now that `butler-apply` and `butler-weekly-review` exist with clearer single-purpose framings, the original butler-briefing and butler-debrief skills feel less useful in practice. Three concrete problems: their ASCII-comic format is dated relative to the more direct slash commands shipped since; the names are too similar to each other (which confuses recall — which one runs at session start, which at session end?); and the actual content overlaps with `butler-weekly-review` enough that having three "give me a Reginald summary" skills is more friction than help. Decide between three options: rename for clarity (e.g. `butler-morning` and `butler-evening` to make the temporal split obvious), redesign the format to match the newer skills' style and trim the ASCII-comic emphasis, or retire one or both in favour of a single consolidated `butler-status` skill that subsumes briefing + debrief + weekly-review behind a `--mode` argument. Tied to the Distributable butler skills entry above — whatever survives this should also be path-genericised in the same pass.
