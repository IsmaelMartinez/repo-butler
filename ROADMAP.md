...
The GitHub API client handles rate limiting with automatic retry/backoff. Branch protection is enabled on main. CI runs 55 tests and secret-leak checks on every PR.
...
### 2. Feed triage bot trends into LLM prompts

The ASSESS phase fetches triage bot synthesis findings (`context.triageBotTrends`) but doesn't yet feed them into the LLM prompt. The assessment and ideation prompts should include cluster detection results, ADR drift signals, and upstream impact findings when available.

### 3. Multi-repo trend charts
