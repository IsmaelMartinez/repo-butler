...
Progress (2026-07-13): scheduled promotion shipped. `code-review-bot` joined the `apply-schedule` allow-list — the last held-back class, completing the ADR-007 stage-4 rollout — backed by the clean 13/13 live rollout plus four subsequent clean weekly scheduled runs. ADR-009 carries the gate analysis ("Scheduled promotion (2026-07-13)" note); the class remains auto-merge-ineligible by construction.
