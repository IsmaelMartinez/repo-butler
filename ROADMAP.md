...
**UPDATE prompt rebuild (prerequisite for re-graduation)** — SHIPPED via PRs #179, #187, #188; soak in progress. PR #179 (2026-05-05) rewrote `buildUpdatePrompt` in `src/update.js` as edit-only instructions ("smallest set of changes", "do not delete or summarise existing SHIPPED records", "Append new entries; do not rewrite old ones"), injected the current date as a literal `Today: YYYY-MM-DD` so `Last Updated:` is deterministic, and added an 80% length-preservation guard (`checkLengthPreservation()`) that refuses the PR if the LLM output drops below that ratio. Regression tests pin the new instruction text and the SHIPPED-marker passthrough. Section-edit mode (LLM emits `{section_heading, action, new_text}` ops applied deterministically) was deliberately deferred — re-evaluate only if the simpler fixes fail to soak cleanly. PR #187 (2026-05-06) added head+tail logging on guard failure for diagnosability. PR #188 (2026-05-06) bumped both providers' max output tokens 4096 → 32768 and disabled Gemini 2.5 Flash thinking on these structured rewrites — the original 4096 cap had been truncating the LLM output mid-document at ~600 chars, tripping the guard on every cron since #179 merged. **Soak window: 2026-05-06 → 2026-05-20** (14 days of clean dry-run output). Verification cron at 2026-05-06 14:01 UTC was the first to pass `safety: passed`. After the window closes, re-flip `self-test.yml`'s `INPUT_DRY_RUN` fallback to `'false'`. Quality monitoring during the soak: the 80% length guard is a binary catastrophe detector only — it does not catch hallucinated dates, fabricated PR numbers, in-place SHIPPED rewrites, or stale-data overwrites from the model's training cutoff. Spot-check the dry-run preview every ~3 days against current `ROADMAP.md`; the log preview was widened in PR (this PR) from `first 500 chars` to head+tail+heading-list+marker counts to make the skim quick.
...
~~**Butler skills consolidation**~~ — SHIPPED 2026-05-06 across four PRs. Closes both predecessor entries — "Distributable butler skills" and "Butler-briefing/debrief refresh or retirement". Plan at `docs/superpowers/plans/2026-05-05-butler-skills-consolidation.md`. End state: two skills shipped from `skills/` in this repo — `repo-butler` (read-side, briefing/debrief modes via positional arg) and `repo-butler-apply` (write-side, confirm-gated). PR #182 hygiene + portability, PR #184 merged briefing+debrief into `repo-butler`, PR #183 renamed apply → `repo-butler-apply`, PR 4 (this entry) the Reginald uplift: dropped the stick-figure for two ASCII silhouettes (bowler+moustache+bow tie for read; silver tray with mood-varying contents for apply), mixed whisky into the closings, added streak awareness, household-member metaphors, recurring grievances, giving-up lore for stale findings, disapproving `*ahem*` for unreviewed merges, and reserved extremis signals (mourning frame, fortnight-rate-limited Burns half-line) — all under a hard line/word budget (≤250 lines body, ≤30 persona, ≤8 closings per mode). Distribution polish (settings-key for project dirs, MCP-first data fetcher, install.sh) is deferred to a separate PR.
