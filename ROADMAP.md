...
~~**Wire ASSESS into the daily schedule**~~ — SHIPPED. `self-test.yml` now defaults to `observe,assess,report`, so the daily run diffs snapshots, populates `context.trends`, and feeds the trend direction into `report.js:231-232`. `schedule.assess: daily` in roadmap.yml is no longer aspirational.
