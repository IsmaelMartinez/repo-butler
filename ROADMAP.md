...
**UPDATE prompt rebuild (prerequisite for re-graduation)** — `buildUpdatePrompt` in `src/update.js:166-226` instructs the LLM to "Update the roadmap to reflect the current state. Keep the same structure and tone" but also to "Be concise — the roadmap should be scannable, not verbose." The LLM resolves that tension by rewriting from scratch, which is the wrong default for a long-form planning document that *is* the project's history. Concrete fixes:

- Replace the "be concise" framing with explicit edit-only instructions: "Make the smallest set of changes needed. Do not delete or summarise existing SHIPPED records, Future entries, ADR references, or section headings. Append new entries; do not rewrite old ones."
- Inject the current date as a literal value in the prompt items (e.g. `Today: ${new Date().toISOString().slice(0, 10)}`) so `Last Updated:` is always set deterministically rather than left to the LLM's training cutoff.
- Add a length-preservation guard in `update.js`: if the generated roadmap is <80% of the input length, log a SAFETY warning and refuse the PR (treat as a destructive rewrite). This is symmetric with the existing `validateRoadmap` safety check.
- Consider switching from "regenerate the whole document" to a section-edit mode (e.g. ask the LLM to emit a list of `{section_heading, action: 'append'|'replace', new_text}` operations applied deterministically), so the document structure is owned by code rather than by the LLM. Bigger change; evaluate after the smaller fixes.
- Add a regression test for the prompt that runs a small fixture roadmap through the prompt-builder and asserts the SHIPPED markers and section headings appear verbatim in the prompt context (separate from end-to-end LLM testing).
- After the prompt change, repeat the soak: 2+ weeks of clean dry-run output, then re-flip `self-test.yml`'s `INPUT_DRY_RUN` fallback to `'false'`.
