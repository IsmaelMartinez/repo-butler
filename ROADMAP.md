...
The phased plan (G1–G11 across six default-closed phases) builds these in dependency order: G1 (this set of ADRs), then the dormant `targetRepo` field, deterministic safety and admissibility gates, routing behind empty allow-lists, the council quality filter and issue format, the soak-then-graduate write phase, and the optional net-new deterministic classes. Status: G1 (ADRs, #296), G2 (dormant `targetRepo` field, #297), G3 (cross-reference autolink neutralisation in `safety.js`, #298), and G4 (the deterministic `resolveCrossRepoDestination` finding-anchoring gate) are shipped; all remain dormant — no routing is wired and no cross-repo write graduates live until G5 wires the gate behind empty allow-lists and a class clears its month-long IDEATE soak.
