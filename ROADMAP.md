...
**UPDATE prompt rebuild (prerequisite for re-graduation)** — IN PROGRESS 2026-05-04. The first pass of fixes lands as the prompt-rebuild PR: `buildUpdatePrompt` in `src/update.js` is rewritten as edit-only instructions ("smallest set of changes", "do not delete or summarise existing SHIPPED records", "Append new entries; do not rewrite old ones"), the current date is injected as a literal `Today: YYYY-MM-DD` so `Last Updated:` is deterministic, and an 80% length-preservation guard (`checkLengthPreservation()`) refuses the PR if the LLM output drops below that ratio of the input. Regression tests pin the new instruction text and the SHIPPED-marker passthrough so a future edit can't silently regress them. Section-edit mode (LLM emits `{section_heading, action, new_text}` ops applied deterministically) is deliberately deferred — evaluate only if the simpler fixes fail to soak cleanly. Cron run that produced PR #178 (closed) confirmed the prior cron-vs-merge race is now mitigated by the dedup guard plus dry-run default. Soak: 2+ weeks of clean dry-run output, then re-flip `self-test.yml`'s `INPUT_DRY_RUN` fallback to `'false'`.
...
**Butler skills consolidation** — IN PROGRESS 2026-05-05. Replaces the two separate "Distributable butler skills" and "Butler-briefing/debrief refresh or retirement" entries with a single 4-PR programme. Plan at `docs/superpowers/plans/2026-05-05-butler-skills-consolidation.md`. End state: two skills shipped from `skills/` in this repo — `repo-butler` (read-side, briefing/debrief modes via positional arg) and `repo-butler-apply` (write-side, confirm-gated). PR #182 (hygiene + portability) is the first of four. PR 2 merges briefing+debrief into `repo-butler`. PR 3 renames apply → `repo-butler-apply`. PR 4 is the Reginald uplift: drops the stick-figure for two new ASCII silhouettes (bowler+moustache+bow tie for read; silver tray with mood-varying contents for apply), mixes whisky into the closings, adds streak awareness, household-member metaphors, and reserved extremis signals (mourning frame, Burns half-line) — all under a hard line/word budget. Distribution polish (settings-key for project dirs, MCP-first data fetcher, install.sh) is deferred to a separate PR after consolidation lands.
