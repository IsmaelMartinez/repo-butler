...
The phased plan (G1–G11 across six default-closed phases) builds these in dependency order: G1 (this set of ADRs), then the dormant `targetRepo` field, deterministic safety and admissibility gates, routing behind empty allow-lists, the council quality filter and issue format, the soak-then-graduate write phase, and the optional net-new deterministic classes. Status: G1 (ADRs, #296), G2 (dormant `targetRepo` field, #297), G3 (cross-reference autolink neutralisation in `safety.js`, #298), G4 (the deterministic `resolveCrossRepoDestination` finding-anchoring gate, #299), G5 (the gate wired into `propose()` behind empty `propose-targets`/`propose-classes` allow-lists, #300), G6 (the per-target two-axis volume cap, #301), and G7 (closed-issue look-back in `findDuplicates`, #302) are shipped; G8 (the council quality filter — a statistic-grounding rule in the deliberation prompts plus a deterministic confidence/priority gate that holds weak cross-repo ideas back to the watchlist, defence-in-depth and never the boundary) is in review. All remain dormant — no routing is wired live and no cross-repo write graduates until a class clears its month-long IDEATE soak.
