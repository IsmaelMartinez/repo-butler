...
describe('buildCrossRepoIssueBody (G9 deterministic body)', () => {
  it('composes a standards-gap body from the finding statistic, never the LLM fields', () => {
    const finding = { type: 'standards-gap', tool: 'dependabot-auto-merge', compliant: ['a', 'b', 'c'], nonCompliant: ['target'], adoptionRate: 0.75 };
    const body = buildCrossRepoIssueBody(
      { title: 'x', rationale: 'LLM_RATIONALE_LEAK', currentState: 'HOST_STATE_LEAK', affectedFiles: ['host/file.js'] },
      finding,
      { trackingUrl: 'https://github.com/IsmaelMartinez/repo-butler/issues/5' },
    );
    ...
    assert.ok(!body.includes('LLM_RATIONALE_LEAK'), 'LLM rationale never composed into a cross-repo body');
    assert.ok(!body.includes('HOST_STATE_LEAK') && !body.includes('host/file.js'), 'host-oriented LLM fields suppressed');
  ...
  it('composes a policy-drift body with the expected/actual comparison', () => {
    const body = buildCrossRepoIssueBody({ title: 'x' }, { type: 'policy-drift', category: 'ci-reliability', expected: '95%', actual: '70%' }, {});
  ...
  it('composes a tier-uplift body listing the named failing checks', () => {
    const body = buildCrossRepoIssueBody({ title: 'x' }, { type: 'tier-uplift', currentTier: 'silver', targetTier: 'gold', failingChecks: [{ name: 'Code scanning' }, { name: 'Secret scanning' }] }, {});
  ...
  it('omits the back-link line when no tracking URL is supplied (dry-run)', () => {
    const body = buildCrossRepoIssueBody({ title: 'x' }, { type: 'policy-drift', category: 'license', expected: 'MIT', actual: 'None' }, { trackingUrl: null });
  ...
  it('never carries a cross-reference autolink — passes validateIssueBody({crossRepo:true})', () => {
    ...
    for (const finding of findings) {
      const body = buildCrossRepoIssueBody({ title: 'x' }, finding, { trackingUrl: 'https://github.com/IsmaelMartinez/repo-butler/issues/5' });
  ...
  it('falls back to a safe portfolio-grounded line for an unknown anchor type', () => {
    const body = buildCrossRepoIssueBody({ title: 'x' }, { type: 'dependabot-stale' }, {});
