...
// The marker written by update.js into roadmap PR bodies. Distinct from
// APPLY_PR_MARKER, which apply.js AND onboard.js both emit.
const ROADMAP_PR_MARKER = 'This PR was generated by [Repo Butler]';
...
function isVerified(pr, prClass) {
  const body = typeof pr?.body === 'string' ? pr.body : '';
  if (body.includes(APPLY_PR_MARKER)) return true;
  if (prClass === 'roadmap-update' && body.includes(ROADMAP_PR_MARKER)) return true;
    ...
    try {
      ...
      for (const [i, s] of stale.entries()) {
        const base = {
          ...
          verified: isVerified(s.pr, s.prClass),
