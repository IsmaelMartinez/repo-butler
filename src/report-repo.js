...
export function generateRepoReport(snapshot, prActivity, issueActivity, prAuthors, trends, openPRs = [], cycleTime = null, weeklyCommits = [], depSummary = null, libyear = null, config = {}, assessment = null) {
...
new Chart(document.getElementById('trendsChart'),{type:'line',data:{labels:[${trends.weeks.map(w => `'${w.week}'`).join(',')}],datasets:[{label:'Open Issues',data:[${trends.weeks.map(w => w.open_issues).join(',')}],borderColor:__C.danger,backgroundColor:__C.danger+'1a',fill:true,tension:0.3,pointRadius:4,yAxisID:'y'}${mergedPrDataset}]},options:{responsive:true,plugins:{legend:{position:'top'}},scales:{y:{type:'linear',position:'left',beginAtZero:true,grid:{},title:{display:true,text:'Open Issues'}}${mergedPrScale},x:{grid:{display:false}}}}});` : '';
  ...
  const liveSite = deployedLink(snapshot.meta?.homepage, 'live site ↗');
...
<div class="subtitle">${fmt(snapshot.meta?.stars)} stars · ${fmt(snapshot.meta?.forks)} forks · ${fmt(snapshot.meta?.watchers)} watchers — ${now} — <a href="index.html">portfolio</a> — <a href="digest.html">digest</a>${liveSite ? ` — ${liveSite}` : ''}</div>
<div class="grid">
...
<div class="subtitle">${escHtml(repo.description || 'No description')} — ${now} — <a href="index.html">portfolio view</a>${liveSite ? ` — ${liveSite}` : ''}</div>
<div class="grid">
