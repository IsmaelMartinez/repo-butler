...
| `weekly-ideate.yml` | cron Mon 06:00 UTC | observe → ideate → propose (dry-run; council deliberates inside ideate) | council verdicts + watchlist + a `propose-soak.json` ledger entry (no issues filed here) |
