...
This stays true through [ADR-007](007-agents-and-execution.md) stage 5 (selective auto-merge, 2026-06-15): auto-merge is a server-side action inside the scheduled Actions runtime, gated by the `apply-automerge` allow-list — not an MCP tool. The MCP mutation surface remains exactly `trigger_refresh`; no merge, write, or apply tool is exposed to agents.
