...
The v1 release shipped (2026-03-29) for external consumers via `uses: IsmaelMartinez/repo-butler@v1`. The butler's primary value is the portfolio view, which requires running from a central repo against many repos. Individual repo owners get more value from the triage bot for per-repo intelligence, but the report-only use case (health dashboards without governance) is still valuable standalone. The butler's consumer story is: "install it in your portfolio's meta-repo and point it at all your repos."
