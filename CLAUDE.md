...
`src/governance.js` runs as a first-class GOVERNANCE phase between UPDATE and IDEATE, producing five finding types — standards gaps, policy drift, tier-uplift proposals, open-vulnerability findings (repos with open critical/high Dependabot/code-scanning alerts, or any secret-scanning hit), and stale-Dependabot-PR audits — which are persisted to the data branch for the MCP `get_governance_findings` tool, the dashboard, and the `governance:apply` workflow. `open-vulnerability` is a per-repo *state* finding (like `dependabot-stale`): it routes to `executor: 'manual'` and is never wired to the templated-PR path or cross-repo PROPOSE (ADR-002/ADR-011 lane boundary — resolving a specific alert is per-repo work, not a cross-repo statistic). Detection is pure deterministic JS (no LLM cost), so the daily pipeline runs it 4×/day to keep findings fresh; the weekly IDEATE run picks up the same fresh findings via `runGovernance`'s idempotency guard inside `runIdeate`.
...
`src/observe.js` gathers data via GitHub REST API. It runs ~13 API calls in parallel via `Promise.all`, including community health profile, Dependabot alerts, code scanning alerts, secret scanning alerts, CI pass rate, and computes derived metrics (bus factor, time-to-close median). `observePortfolio()` classifies all repos by activity level. Repo discovery tries `/installation/repositories` (GitHub App token), falling back to `/user/repos` (PAT), then to the public-only `/users/{owner}/repos` and `/orgs/{owner}/repos`. Private repos returned by the privileged endpoints are returned separately as `portfolio.privateRepos`, never mixed into `portfolio.repos` — see the private-repo monitoring section below.

## Private-repo monitoring (monitored, never published)

**repo-butler is itself a public repo, so it has three world-readable sinks, not one:** the GitHub Pages dashboard, the `repo-butler-data` snapshot branch, and the Actions run logs. A private repo name reaching any of them is a permanent disclosure — caches and forks make it unfixable after the fact. Treat all three as equally public.

Private repos are therefore monitored but withheld, using a default-safe split rather than per-writer filtering:

- `observePortfolio()` returns `repos` (public only, unchanged shape) and `privateRepos` (private, tagged `private: true`). **Every pre-existing consumer reads `repos`**, so the report HTML, the weekly/latest snapshots, the repo cache, the GITHUB_OUTPUT summary and cross-repo PROPOSE cannot leak a private name by accident and needed no changes. `classification` is computed from public repos only, because it feeds the GITHUB_OUTPUT summary.
- Only two consumers opt in to private data by reading `privateRepos` explicitly: governance detection and the health-tier/uplift check that rides along with it. This inverts the failure mode — the risk is no longer "every writer must remember to filter" but "a writer must deliberately ask for private data".
- `runGovernance` runs all detectors over `[...repos, ...privateRepos]`, then tags private-repo findings with `private: true`. That flag is the single carrier of the redaction decision.
- `store.publishableFindings()` is the **sink-level** defence, deliberately duplicating the split above: `writeGovernanceFindings` and `writeGovernanceWeekly` withhold `private` findings before writing. It **fails closed** — a non-boolean truthy `private` value is withheld, not published. A future caller that forgets to filter still cannot leak; publishing requires deliberately stripping the flag.
- `src/private-notify.js` is the delivery channel, because withholding findings from every public sink without delivering them anywhere would mean private repos are monitored in name only. It upserts **one tracking issue on the private repo itself** (`Repo Butler: open governance findings`), rewritten in place each run, and closes it once that repo has no findings left. Private-to-private, so nothing leaks. It defaults to `dryRun: true`.
- **Every log line touching private repos counts them and never names them** — that is a hard rule, not a style preference, because Actions logs are public. `private-notify.js` also deliberately swallows error text rather than logging it, since GitHub error bodies echo the request path (which contains the repo name).

A useful consequence of enforcing redaction at the sink: the GOVERNANCE APPLY phase reads findings via `store.readGovernanceFindings()` (`index.js`), so it can never see a private-repo finding and therefore never opens a remediation PR on a private repo or names one in its ~21 repo-interpolating log lines. Private repos get detection and notification but not automated remediation — deliberate, since apply's logs are public. Remediating a private repo stays a human action taken from its own tracking issue.

When adding a new writer, read `repos` unless you have a specific reason not to. When adding a new sink that persists findings, route it through `publishableFindings()`. The guard tests live in `observe.test.js` ("never mentions a private repo name in the publishable payload" — it serialises the payload and asserts the name is simply absent, so a newly-added field carrying private data is caught) and `store.test.js` ("governance writers withhold private findings from the data branch").
